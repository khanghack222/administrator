...
export async function testAllApi(opts = {}) {
  const domain = opts.domain || "iunp.edu.rs";
...
/** Tạo temp mail.tm — nhận OTP GetEduMail, không dính guest limit. */
export async function createTempMailbox(log = () => {}) {
  const dRes = await fetch(`${MAILTM}/domains`);
  if (!dRes.ok) throw new Error(`mail.tm domains ${dRes.status}`);
  const dJson = await dRes.json();
  const domain =
    dJson["hydra:member"]?.[0]?.domain ||
    dJson[0]?.domain ||
    dJson.domain;
  ...
  const accText = await r.text();
  if (!r.ok) throw new Error(`mail.tm account ${r.status}: ${accText.slice(0, 120)}`);
  ...
  if (!r.ok || !tok.token)
    throw new Error(`mail.tm token ${r.status}: ${JSON.stringify(tok).slice(0, 120)}`);

  log(`[temp] ${address}`);
  return { address, password, token: tok.token, domain };
}

/** Poll mail.tm inbox lấy mã 6 số (OTP GetEduMail). */
export async function waitTempCode(tm, { timeoutMs = 90_000, onTick } = {}) {
  const t0 = Date.now();
  let i = 0;
  while (Date.now() - t0 < timeoutMs) {
    ...
    const list = j["hydra:member"] || (Array.isArray(j) ? j : []);
    for (const m of list) {
      ...
      const blob = [
        ...
        full.intro,
        full.text,
        typeof full.html === "string" ? full.html : JSON.stringify(full.html || ""),
      ].join("\n");
      const code =
        extractCode(blob) ||
        (blob.match(/\b(\d{6})\b/) || [])[1] ||
        null;
      if (code) return { code, subject: full.subject || "" };
    }
    await sleep(2000);
  }
  throw new Error("temp mail OTP timeout");
}

/**
 * Tạo edu mail qua temp mail.tm (bypass guest limit GetEduMail).
 * Flow: mail.tm → register GetEduMail(temp) → OTP → claim @domain edu.
 */
export async function createAccount(opts = {}) {
  ...
  // password cho GetEduMail account (= login web); edu inbox dùng cùng token
  const password = opts.password || randPass();
  const log = opts.log || (() => {});

  log(`[1] Temp mail (mail.tm)`);
  const tm = await createTempMailbox(log);

  log(`[2] Register GetEduMail = ${tm.address}`);
  let r = await api("POST", "/getedumail/user/register", {
    body: { name: fullName, email: tm.address, password },
  });
  ...
  log(`[4] Wait OTP trên temp…`);
  const { code } = await waitTempCode(tm, {
    onTick: (i) => log(`[4] temp poll ${i + 1}`),
  });
  log(`[4] Code ${code}`);
  ...
  for (const body of [
    ...
    r = await api("POST", "/getedumail/emails", { token, body });
    if (r.ok || r.status === 201) {
      claimed = { path: "/getedumail/emails", body, r };
      break;
    }
  }
  if (!claimed) {
    r = await api("POST", "/getedumail/emails/claim", {
      token,
      body: { email: eduEmail },
    });
    if (r.ok || r.status === 201) claimed = { path: "claim", r };
  }
  if (!claimed) throw new Error(`claim edu ${r.status}: ${r.text}`);
  log(`[6] Claimed via ${claimed.path}`);

  const guestId = claimed.r?.json?.id || null;
  const result = {
    ok: true,
    email: eduEmail,
    fullName,
  ...
  log(`[saved] acc/${id}.json  edu=${eduEmail}`);
